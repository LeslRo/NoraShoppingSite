...
(function(){
	...
	myLib.get = function(param, successCallback) {
		...
		myLib.processJSON('index-process.php?' + encodeParam(param), null, successCallback);
	};
	// To send an action to the admin-process.php over AJAX
	myLib.post = function(param, successCallback) {
		myLib.processJSON('index-process.php?rnd=' + new Date().getTime(), param, successCallback, {method:'POST'});
	};
	...
	myLib.submit = function(form, successCallback) {
		myLib.validate(form) && myLib.ajax({
			method: 'POST',
			url: form.getAttribute('action') || 'index-process.php',
    ...
    myLib.postK = function(param, successCallback) {
        myLib.processJSON('index-process.php?rnd=' + new Date().getTime(), param, successCallback, {method:'POST'});
    };

    myLib.getK = function(param, successCallback) {
        param = param || {};
        param.rnd =  new Date().getTime(); // to avoid caching in IE
        myLib.processJSON('getSingleByPid.php?' + encodeParam(param), null, successCallback);
    };
})();